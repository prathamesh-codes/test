...
import { ProductDetailsContainer } from "../../containers";
    ...
    <div className="item-card" key={index}>
      <div className="item-image">
        ...
        <div className="rating-detail">
          {item.rating} <i class="fas fa-star star"></i> |{" "}
          {nFormatter(item.numberOfReviews)}
        </div>
        <div>
          <ViewSimilarButton
            item={item}
            externalClassName="view-similar-mobile-button"
          />
        </div>
      </div>
      <Link to={`/${item.id}`}>
        <div className="item-info">
          <div className="brand-whishlist">
            <p className="brand-name">{item.brandName}</p>

            <div className="item-action">
              <ViewSimilarButton
                item={item}
                externalClassName="item-card-similar-button"
              />
              <button
          ...
          <p className="price-details">
            ...
            <span className="discount">
              ({item.discountPercent}% <span className="off">OFF</span>)
            </span>
          </p>
        </div>
      </Link>
